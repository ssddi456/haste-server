    ...
    $.ajax('/list', {
      ...
      success: function (res) {
        callback({
          ...
          key: key,
          language: lang,
          lineCount: res.data.list.length
        });
      }
    });
    return;
...
haste.prototype.lightKey = function () {
  this.configureKey(['list', 'new', 'save']);
};

// Show the full key
haste.prototype.fullKey = function () {
  this.configureKey(['list', 'new', 'duplicate', 'twitter', 'raw']);
};
  ...
  this.buttons = [
    {
      $where: $('#box2 .list'),
      label: 'List',
      shortcutDescription: 'control + l',
      shortcut: function (evt) {
        return evt.ctrlKey && (evt.keyCode === 76);
      },
      action: function () {
        location.href = '/list';
      }
